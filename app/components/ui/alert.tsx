...
const alertVariants = cva(
  'relative bg-white w-full rounded-2xl border border-border p-4 shadow shadow-foreground/10',
  {
    variants: {
      variant: {
        default: '',
  ...
  return (
    <View role='alert' className={alertVariants({ variant, className })} {...props}>
      <View className='absolute left-3.5 top-4 -translate-y-0.5'>
        <Icon
          size={iconSize}
          color={variant === 'destructive' ? colors.notification : variant === 'success' ? 'green' : colors.text}
        />
      </View>
      {children}
    </View>
